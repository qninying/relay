...
- [x] STORY-003: handle API calls with circuit breaker and validation
  - Date: 2026-09-22
  - Session: CC-20260921-r1ay
  - What changed: `circuitBreaker.ts`/`withReliability.ts` ported from CoreOps's proven `mcp-server/src/reliability/` pattern (closed/open/half-open state machine, sliding failure window, explicit timeout, capped exponential-backoff retry, injectable clock for deterministic tests). `persistOutput.ts` gives the existing `guardrails/outputContractGuardrail.ts` its first real caller — validates before persisting (REQ-012), refuses to persist on any violation, logs either way. `callExternalApi.ts` composes both: closed breaker lets the call proceed (criterion 1), open breaker blocks it fail-fast without invoking the call at all (criterion 2), and the call's output is validated before persisting (criterion 3/trust) — tested as one flow, not just each piece alone. `src/tsconfig.json`'s `rootDir` changed to `..` with the guardrail file added to `include`, the same cross-package relative-import pattern CoreOps already uses.
  - Verification: `npx vitest run` — 48/48 passing (14 breaker/reliability, 4 persistOutput, 5 composed), all with fake timers so no real waiting. `npx tsc --noEmit` clean. Confirmed the guardrails package's own suite (16/16) is unaffected by the cross-package wiring.
  - Notes: `.colaberry/progress.json` updated, all 3 criteria `true`. `.colaberry/enrichment/STORY-003.json` written. Confidence ~90% — the one open gap: `callExternalApiAndPersist` has only been exercised against a mock `apiCall`, never a real external API, since none exists in this project yet.

- [x] STORY-004: implement dead-letter queue with triage runbook
  - Date: 2026-09-22
  - Session: CC-20260921-r1ay
  - What changed: `dlq.ts` reuses `RedisStreamQueue` (STORY-002) pointed at a separate DLQ stream rather than a new queue mechanism — `moveToDeadLetterQueue()` preserves the original job's payload alongside the failure reason and timestamp; `triageNextDeadLetterEntry()` reads it back and logs `job_triaged` with the reason. Wired into `callExternalApiAndPersist` (STORY-003): a job whose retries are genuinely exhausted (`UpstreamCallFailedError`) moves to the DLQ (criterion 1); a circuit-open rejection deliberately does not, since the call was never attempted — verified both directions against the real Redis DLQ stream. `docs/runbooks/DLQ_TRIAGE.md` written: what lands here and why, how to check it, a decision table by failure reason, when to escalate rather than guess, and an honest list of what it doesn't cover yet.
  - Verification: `npx vitest run` — 55/55 passing (5 `dlq.test.ts`, 2 `callExternalApiDlq.test.ts`, both against the real Redis container). `npx tsc --noEmit` clean. STORY-003's original 5 `callExternalApi.test.ts` tests still pass unmodified — `dlqOptions` is optional, fully backward compatible. One real bug caught and fixed during review: DLQ integration tests initially shared one stream without each test draining what it enqueued, so a later test's `dequeueOne()` picked up an earlier test's leftover entry (Streams read oldest-first) — fixed by having each test drain what it produces, same balance convention `queue.test.ts` already follows.
  - Notes: `.colaberry/progress.json` updated, all 3 criteria `true`. `.colaberry/enrichment/STORY-004.json` written. Deliberately not built yet, named honestly in the runbook: automated retry-from-DLQ (STORY-008), DLQ size/age alerting (STORY-005/STORY-010), any UI.

- [x] STORY-005: display live SLO dashboard
  - Date: 2026-09-22
  - Session: CC-20260921-r1ay
  - What changed: `metrics.ts` — `MetricsCollector` tracking success rate, p95 latency, and a real error budget over a rolling count-based window (REQ-006), wired into `processJob.ts` so it updates from real job outcomes, every recorded metric logged with its correlation ID. `dashboardServer.ts` + `dashboard.html` serve a live dashboard (REQ-018) over a small standalone Node `http` server — no framework. Rebuilt once after direct user feedback the first pass "looks cheap": restructured into a KPI summary layer (6 real metrics), an operational-detail layer (error budget progress bar, a real recent-activity sparkline), and a needs-attention layer, with real states (healthy/warning/critical/no-data/offline) and reusable card-rendering functions instead of hand-copied markup. Wired real DLQ depth from Redis (`XLEN`) into the dashboard via an injected provider — `null`, never a fabricated `0`, when Redis isn't reachable. `runDashboard.ts` feeds real simulated activity through the real `processJob()` and occasionally a real DLQ entry, for demoing/manual verification.
  - Verification: `npx vitest run` — 76/76 passing. `npx tsc --noEmit` clean. Watched it running live in the browser at desktop, tablet, and mobile widths (not just screenshots — confirmed `window.innerWidth` and computed `grid-template-columns` actually changed), zero console errors. Two real bugs found and fixed in review: a raw negative error-budget percentage (`-173%`) displaying instead of clamping to a 0% floor, and a missing `<meta name="viewport">` tag that silently broke all responsive behavior on mobile (pages without it render at a 980px virtual viewport regardless of physical screen width — caught by checking `window.innerWidth` directly after a screenshot looked suspiciously non-responsive, not by assuming the CSS was right).
  - Notes: `.colaberry/progress.json` updated, all 3 criteria `true`. `.colaberry/enrichment/STORY-005.json` written. Confidence ~85% — the dashboard has only ever shown simulated activity; it's never been pointed at Relay's real pipeline processing real jobs, since nothing wires a running worker to this `MetricsCollector` yet.
